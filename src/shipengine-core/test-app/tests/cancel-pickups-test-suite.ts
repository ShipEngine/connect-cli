        ...
        for (let deliveryService of carrierApp.deliveryServices) {
          for (let packageUnit of packageUnits) {
            for (let packageWeight of packageWeights) {
              ...
              const title = `cancels a pickup with pickup service: ${pickupService.name}, delivery service: ${deliveryService.name}, package unit: ${packageUnit}, package weight: ${packageWeight}, and reason ${reason}`;
