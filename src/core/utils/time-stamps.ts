...
interface TimeStampsCache {
  [timeZone: string]: TimeStamps
}

let timeStampsCache: TimeStampsCache = {};
...
export function initializeTimeStamps(timeZone: string): TimeStamps {
  ...
  if(timeStampsCache[timeZone]) {
    return timeStampsCache[timeZone];
  }
  else {
    let date = new Date();
    ...
    timeStampsCache[timeZone] = timeStamps;
